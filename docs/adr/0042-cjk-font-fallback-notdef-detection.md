...
### 2. 責務の分割：Core がコードポイント → ファミリ名、アダプタがファミリ名 → URL
...
アダプタ（JS）: FONT_URL_MAP = { "Noto Sans JP": "https://…" }
  └─ CDN URL（デプロイ環境依存、変更頻度が高い）
```

Core は `.notdef` グリフを検出した時点でコードポイント → ファミリ名の変換を行い、`FetchFont { family: String }` イベントを発火する。URL は知らない。

**`FontMissing { codepoints: Vec<u32> }` を却下した理由**
コードポイント → ファミリ名のマッピングはドメイン知識であり、すべてのプラットフォームアダプタで同じロジックを重複実装することになる。Core が Unicode ブロックテーブルを持つことで、アダプタは URL マッピングだけを知ればよい。

**Flutter 方式（Core が CDN URL を持つ）を却下した理由**
Core にネットワーク依存とハードコード URL を持ち込むと、オフライン・イントラネット・CDN 変更に脆くなる。URL はアプリ層の責務とする。

### 3. 通知：`FetchFont { family }` イベントを `poll_events()` 経由で JS に渡す
