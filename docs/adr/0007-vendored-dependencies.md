...
Vello・Taffy・parley・fontique・skrifa を `crates/vendor/` として workspace に取り込み、upstream との依存関係を切断する。upstream の変更に引きずられず、NewDOM の都合で任意のタイミングに upstream の改良を選択的に取り込む。
...
upstream から cherry-pick する運用が必要。ただし「取り込むかどうか」の判断権は常に NewDOM 側にある。
